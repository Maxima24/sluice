...
import { totalOutbound } from '@/lib/liquidity';
...
export function RouteSimulation() {
  ...
  const sendable = totalOutbound(channels);
  const ready = channels.length > 0 && BigInt(sendable) > BigInt(0);
  const statusCopy = {
  ...
  return (
    <div className="flex h-full min-h-[260px] flex-col gap-4 bg-transparent">
      ...
      <MetricStrip
        items={[
          ['SENDABLE', channels.length ? formatCkb(sendable) : '—'],
          ['PEERS', peers.data ? String(peers.data.length) : '—'],
          ['CHANNELS', String(channels.length)],
          ['STATUS', health.isError ? 'offline' : ready ? 'ready' : 'blocked'],
        ]}
      />
    </div>
  );
