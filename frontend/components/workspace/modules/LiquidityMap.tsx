...
export function LiquidityMap() {
  const [selected, setSelected] = useState('chan-03');
  const rows = [
    { id: 'chan-01', outbound: 78, inbound: 42, state: 'balanced' },
    { id: 'chan-02', outbound: 36, inbound: 68, state: 'thin outbound' },
    { id: 'chan-03', outbound: 18, inbound: 86, state: 'critical' },
    { id: 'chan-04', outbound: 64, inbound: 52, state: 'recovering' },
  ] as const;
    ...
    <div data-no-magnetic className="grid h-full min-h-[290px] grid-cols-[minmax(0,1fr)_140px] gap-5 bg-transparent">
          ...
          {rows.map((row, index) => (
            <button
              ...
              className={cn(
                ...
                selected === row.id ? 'border-white/35' : '',
              )}
            >
      ...
      <div className="border-l-2 border-white/30 pl-3">
        ...
        <div className="mt-5 space-y-5">
          <Meter label="OUTBOUND" value={active.outbound} />
          <Meter label="INBOUND" value={active.inbound} muted />
        </div>
      </div>
    </div>
  );
